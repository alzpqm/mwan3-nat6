...
- 延續 0.0.48 的 LuCI 修正；真機瀏覽器另發現健康摘要會顯示字面值
  `null`，修正空白錯誤區塊，並新增回歸測試。
- 測試渲染器改為保留 LuCI 對陣列中 null 子元素的行為；前端行為測試
  增至十組。0.0.48 候選套件不安裝、不發布。

## 0.0.48 - 2026-09-19

- 修正 LuCI 狀態頁摘要卡使用 `header` 標籤而繼承 Bootstrap 頂部導覽的
  高 z-index，蓋住 OpenWrt 下拉選單的問題；改用獨立區塊並限制內部堆疊。
- WAN 設定的間隔、樣本數、PD 長度與位址／前綴索引必須為整數，避免
  LuCI 接受小數但核心拒絕套用。
- 唯讀帳號停用手動套用，失敗狀態停用按鈕；停用本機診斷但仍殘留規則時
  顯示需要處理，不再誤報所有線路正常。
- 自動監看僅顯示已啟用設定，不再據此宣稱服務運作中；補齊 RPC／缺少
  核心程式的正體中文錯誤說明與英文翻譯。
- 新增九組前端行為測試，以及使用官方 Bootstrap CSS 的桌面／手機選單
  遮擋回歸測試頁面與本機預覽工具。

## 0.0.47 - 2026-09-05

- 延續0.0.46已通過的本機、SDK與實機內容；0.0.46發布前隱私檢查把
  公開指南內通用的root備份範例誤判為私人實機證據路徑，故在GitHub寫入前
  封存。
- 隱私檢查只拒絕帶版本的實機rollback／stage形式與真實部署識別資料，保留
  使用者可直接照做的通用備份／回復範例。

## 0.0.46 - 2026-09-05

- 延續0.0.45的產品內容；0.0.45第一個本機gate在執行產品測試前，因測試
  編排把多檔 `grep -c` 輸出誤作單一整數而停止，故封存並順延版本。
- 版本一致性檢查改為逐檔精確比對；核心、LuCI與語言包仍須重新完成所有
  本機、SDK及實機gate。

## 0.0.45 - 2026-09-05

- 延續0.0.44已通過的核心、LuCI儀表板與翻譯內容，不變更NAT、RPDB、
  watcher、mwan3或RPC行為。
- 0.0.44的core與LuCI實機gate通過；English安裝後驗證誤用目標映像未提供
  的 `stat`，回復English與LuCI設定後封存該版本。本版以BusyBox現有介面
  驗證語言包，不再假設安裝coreutils。

## 0.0.44 - 2026-09-05

- 延續0.0.43已通過的LuCI、翻譯與本機測試內容，不變更任何執行期邏輯。
- 0.0.43的精確來源已傳到Debian，但過寬的全倉庫POSIX ShellCheck把
  OpenWrt BusyBox ash支援的 `local` 視為SC3043並使正式gate失敗，因此
  在建置前封存。
- Debian靜態gate限於實際封裝到路由器的shell payload，並明確排除已審查
  的OpenWrt ash SC3043；build/test helper仍由各自的語法與完整測試覆蓋。

## 0.0.43 - 2026-09-05

- 延續0.0.42的LuCI狀態儀表板，不變更NAT、RPDB、watcher、mwan3或
  Apply安全邏輯。
- 補齊摘要卡獨立文字「未啟用」的英文翻譯；0.0.42在任何建置、安裝或
  發布前被語言完整性測試封存。

## 0.0.42 - 2026-09-05

- 將 LuCI 狀態頁重整為清楚的健康總覽、四張摘要卡、responsive WAN 卡片、
  規則／自動化資訊區及分流說明，讓桌面與手機都能快速判讀多 WAN 狀態。
- 所有新樣式都限定在本套件頁面內，並以文字搭配色彩呈現狀態；不新增
  RPC、ACL、UCI 權限或外部前端相依。
- 保留 0.0.41 的 NAT、RPDB、watcher、mwan3 與 Apply 安全邏輯，只更新
  LuCI 顯示、自然正體中文及獨立英文語言包。
